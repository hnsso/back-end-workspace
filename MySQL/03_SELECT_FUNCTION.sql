...
/*
    ...
    * 날짜 단위 : YEAR(연도), QUARTER(분기), MONTH(월), WEEK(주), DAY(일), HOUR(시간)
				MINUTE(분), SECOND(초)
*/
SELECT period_diff(202312, 202403); -- -3
SELECT period_diff(202403, 202312); -- 15
...
SELECT emp_name, hire_date, 
	   -- 밑에랑 같은거다 !!datediff(now(),hire_date) "근무 일 수"
...
SELECT emp_name, hire_date
	adddate(hire_date, interval)
    
    /*
		LART_DATE(날짜) : 해당 월의 마지막 날짜를 반환
    */
    SELECT last_day(now());
    ...
    */
    ...
    SELECT emp_name, bonus, if(bouns,0.7 ,0.1)
    FROM employee;
...
SELECT dept_code,
	   emp_name,
       if((substr(emp_no, 8, 1) = 1), '남자', '여자')
FROM employee;



-- 사원명, 직급 코드(jobe_code), 기존 급여(salary), 인상된 급여 조회
-- 정렬 : 직급 코드 J1부터, 인상된 급여 높은 순서대로
-- 직급 코드가 J7인 사원은 급여를 10% 인상
-- 작급 코드가 J6인 사원은 급여를 15% 인상
...
SELECT emp_name,
	   jobe_code,
       salary,
	   if(jobe_code = 'J7', salary*1.1, 
			if(jobe_code = 'J6', salary*1.15,
				if(jobe_code = 'J5', salary*1.2, salary * 1.05) 
				) 	
			) "인상된 급여"
FROM employee
...
/*
	그룹함수 --> 결과값이 1개 !
    - 대량의 데이터들로 집계나 통계 같은 작없을 처리해야 하는 경우 사용되는 함수들
...
/*
    ...
    - 모든 그룹 함수는 NULL 값을 자동으로 제외하기 때문에
	  AVG 함수를 사용할 때는 COALESCE 또는 IFNULL 함수와 함께 사용하는 것을 권장한다.
...
*/
-- 가장 작은 값을 해당하는 사원명(emp_name), 급여(salary), 입사일(hire_date)
