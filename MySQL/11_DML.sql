...
-- emp_name 컬럼에 not null 제약조건을 인해 에러 발생
...
-- emp_salary에서 선동일 사장의 급여를 70000000원으로 , 보너스를 0.2로 하락
...
-- 모든 사원의 급여를 기준 급여에서 10프로 인상한 금액(기존 급여 * 1.1)으로 변경
...
SET emp_id = null
WHERE emp_id = 200;
-- > 해당 컬럼에 대한 제약조건이 위배되면 수정이 안된다 ! x
