...
*/
-- 각 부서별 조회
-- SELECT에는 같은 직계와 그룹별로 나와야만 와야한다. 
SELECT dept_code, 
	   count(*) "사원 수",
	   sum(salary) "총 급여",
...
FROM employee
GROUP BY dept_code
ORDER BY dept_code; --- 1  -- 오더바이는 제일 마지막으로 온다 !!정렬하는!!
...
-- 그룹함수에 조건을 걸고 싶으면 무조건 having에다가 조건을 넣는다 !
...
-- count(bonus) 는 그룹 함수이다

/*  ----- SQLD(시험)에서만 나온다 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 rollup|cube(컬럼1, 컬럼2) (cube는 MySQL X) - 실제 볼일은 없는 집계 함수
    - 그룹별 산출한 결과 값의 중간 집계를 계산 해주는 함수
    - rollup : 컬럼1을 가지고 다시 중간집계를 내는 함수 (그룹별 중간 집계가 계속일어난다.)
    - cube : 컬럼1을 가지고 중간집계도 내고, 컬럼2를 가지고도 중간집계를 냄 (큐브는)
...
/*
    ...
    UNION (합집합) : 두 쿼리문 수행한 결과값을 하나로 합쳐서 추출 (중복되는 행 제거)
...
-- 위 쿼리문 대신 where 절에 or 연산자를 사용해서 처리 가능해서 위에껄 안쓴다 !
SELECT emp_id, emp_name, dept_code, salary
FROM employee
