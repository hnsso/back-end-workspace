/*
    ...
    - SQL 종류
	  ...
	  3 . DCL(DATE CONTROL LANGUAGE) : 테이터 제어어
...
SELECT *
FROM department;
-- 4 . employee 테이블의 직원명(emp_name), 이메일 (emaill),
...
/*
	리터럴
    - SELECT 절에 리터럴을 사용하면 테이블에 존재하는 데이터처럼 죄회가 가능
...
-- 유의사항 !! DISTINCT 는 SELECT절에 딱 한번만 기술 가능
...
*/
-- employee 에서 부서코드 (dept_code)가 'D9'인 사원들만 조회
-- (이때, 모든 컬럼 조회)

SELECT * /* 컬럼명*/
FROM employee /*테이블*/
WHERE dept_code = 'D9'; /*조건*/
...
/*
    ...
    비교대상컬럼 LIKE '%문자%' => 비교대상컬럼값이 문자로 "포함"되는걸 조회 (!!키워드 검색!!)
...
-- 전화번호의 3번째 자리가 1인 사원들의, 사원명, 전화번호, 이메일 조회
SELECT emp_id, emp_name, phone, email
FROM employee
WHERE phone LIKE '__1%';

-- 이메일 중 _ 앞글자가 3글자인 사원들의 사번, 이름 , 이메일 조회
-- ex) sim_bs@sh.or.kr, sun_di@kh.or.kr
...
/* 0.()
   ...
   7. OR
*/
-- 1. 직급코드(job_code)가 J7이거나J2인 사원들 중 급여(salary)가 
...
WHERE  manager_id IS NULL AND  dept_code IS NULL ;
	  /*  사수가 없고 그리고 부서배치도 받지 않은 */
      
-- 3. 연봉(보너스포함x)이 3000만원 이상이고 보너스를 받지 않은 사원들의
--    사번(emp_id), 사원명, 급여, 보너스(bonus) 조회
SELECT emp_id, emp_name, salary, bonus
FROM employee
WHERE bonus IS NULL AND salary >= 3000 ;
   --  salary*12>= 3000000 AND bonus IS NULL --

-- 4. 입사일이 '1995-01-01' 이상이고 부서배치 받은 사원들의 
--   사번, 사원명, 입사일(hire_date), 부서코드 조회
SELECT emp_id, emp_name, hire_date, dept_code
FROM employee
WHERE hire_date >= '1995-01-01' AND dept_code IS NOT NULL;

-- 5. 급여가200만원 이상 500만원 이하이고 입사일이 '2001-01-01' 이상이고
--    보너스를 받지않은 사원들의 사번, 사원명, 급여, 입사일, 보너스 조회 
SELECT emp_id, emp_name, salary, hire_date, bonus
FROM employee
WHERE bonus IS NULL AND salary BETWEEN  2000000 AND 5000000 AND hire_date >= '2001-01-01';
      /* salary BETWEEN 2000000 AND 5000000 AND bouns IS NILL */
...
LIMIT
 ...
 -- 행수를 제한하는 MYSQL 전용 비표준 구문
 -- 데이터 양을 제한하고자 할 때 유용
*/
-- 연봉이 높은 5명의 사원의 사원명, 급요 조회
...
-- 페이징 처리! (때문에 LIMIT 사용) LIMIT 절은 두 개의 값이 있을 수 있음!
-- 첫번째 값은 오프셋(offset, 0부터 시작) 시작 행을 지정,
-- 두번째값은 반환할 최대 행 수를 지정
