...
public class A_ArrayList {
	 ...
	 * - 자료구조 : 데이터를 효율적으로 이용하는 방법으로 데이터에
	 * 					편리하게 접근하고, 효율적으로 사용하기 위해서 데이터를 저장하거나 조직하는 방법
	 * 
	 * - Java.util 패키지에 컬렉션과 관련된 인터페이스와 클래스들이 포함 !!! 
	 * - 컬렉션을 사용하면 데이터를 추가, 삭제, 정렬 등의 처리가 간단하게 해결되어 자료구조적 알고리즘을 구현할 필요가 없음
	 * 
	 * * 컬렉션의 장점 
	 * - 저장하는 크기의 제약이 없다 . 
	 ...
	 * 					한시점에서 하나의 스레드만 사용할 수있도록 하는 것을 말한다.
	 * 
	 * 
	 * 
	 */
	public static void main(String[] args) {
		A_ArrayList a = new A_ArrayList(); // 객체 생성 !
//		a.method1();
//		a.method2();
		a.method3();
	}
...
//				-> 내부적으로 기존에 있는 것들은 뒤로 땡기고 해당 인덱스에 값 추가
		...
		System.out.println("사람 수 : " + list.size()); // lenght 대신에 size 1
		...
		for (int i = 0; i < list.size(); i++) {
			System.out.println("이름만 출력 > " + list.get(0).getName());
		}
...
//		for (int i = 0; i < list.size(); i++) {
//
//			if (list.get(i).getAddress().equals("삼성동"))
...
//				System.out.println("삼성동에 사는 사람 > " + list.get(i));
	...
	public void method3() {
		...
		// 11. 오름차순 정렬 : Comparable 인턴페이스를 구현하고 있는 요소를 가지고 비교한 값들 반환 하여 정렬
		Collections.sort(list);

		// 12. 내림차순 정렬 : sort 메서드가 통해서 오름차순 정렬 후 reverse 메서드를 호출하여 정렬
