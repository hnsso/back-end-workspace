...
public class Application {
	...
	public static void main(String[] args) {
		...
		app.mainMenu(); // 메인 메뉴만 호출 !!
	}

	public void mainMenu() {
		/*
		 * 회원 수가 3명이 최대 등록 가능 3명 모두 등록되면 "회원 수가 모두 꽉 찼기 때문에 일부 메뉴만 오픈 됩니다." 와 함께
	...
	public void insertMember() {
		/*
		 * 아이디를 입력 받았는데 기존 맴버 배열에 아이디가 있는 경우 "중복된 아이디입니다. 다시 입력해주세요." 출력 후 다시 아이디 입력부터
		 * 나올 수 있게 처리
		 */
	...
	public void updateMember() {
		/*
		 * 아이디를 입력 받았는데 기존 맴버 배열에 아이디가 없는 경우 "회원 정보가 없습니다." 출력 후 다시 메인 화면으로
		 */
		System.out.println("1. 수정할 회원 아이디 : ");
		String id = sc.nextLine();
		if (mc.checkId(id) == -1) {
			System.out.println("회원 정보가 없습니다");
			return;
		}
		System.out.println("2. 수정할 이름 : ");
		String name = sc.nextLine();
