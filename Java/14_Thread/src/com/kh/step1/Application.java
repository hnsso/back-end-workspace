 ...
 * - 모든 프로세스는 하나 이상의 스레드를 가지며(하나하나의 프로젝트) 각각 독립적인 작업단위를 가진다.
 * - 하나의 스레드는 하나의 코드 실해 흐름이기 때문에 한 프로세스 내에
 * 	 여러개의 스레드가 존재한다면 여러 개의 실행 흐름이 생긴다는 의미
 * 
 * * 메인 스레드
 * - 모든 자바 프로그램은 메인 스레드가 main() 메서드를 실행하면서 시작
 * - main() 메서드의 척 코드부터 아래로 순차적으로 실행되고.
 ...
 * - 각 쓰레드가 효율적을 고르게 실행될 수 있게 해야 한다 !!!!!!!!!
 * 
 * 
 * */
	...
	public static void main(String[] args) {
		...
		// 2 . start를 호출해야한다 그래야 run메서드에 작성한게 시작된다
		go.start();
		come.start();
